...
static ssize_t my_dev_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	for (i = 0; i < maxdatalen; i++){
		switch_value = *PIO_ptr;
		buffer[i] = switch_value;
		pr_info(DEVNAME ": Data Reading 0x%02X\n", switch_value);
	}

	unsigned long ret = copy_to_user(buf, buffer, maxdatalen);
	if (ret){
		pr_err(DEVNAME ": ERROR: copy_to_user failed\n");
		return -EFAULT;
	}
	*offset += maxdatalen;
	return maxdatalen;
...
static int init_handler(struct platform_device *pdev)
{
	...
	// create device class
	// class leds is a reserved class name and can not be used. Therefore we use switches
